...
trainModel = input("Do you want yo use the LBPH algorithm or the Eigen algorithm ? \nType 'LBPH' or 'Eigen' for "
                   "whichever algorithm you want to use : ")

while True:
    if trainModel.lower() == "lbph":
        model = cv2.face.LBPHFaceRecognizer_create()
        model.train(training_images, training_labels)
        print("Please be patient while the images are being analyzed !")
        break

    elif trainModel.lower() == "eigen":
        model = cv2.face.EigenFaceRecognizer_create()
        model.train(training_images, training_labels)
        print("Please be patient while the images are being analyzed !")
        break

    elif trainModel.lower() != "eigen" or trainModel.lower() != "lbph":
